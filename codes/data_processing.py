...
def standardizeData(X_train,X_test,mode):
    tr_arrays,tr_rows,tr_cols = X_train.shape
    ##print((tr_arrays,tr_rows,tr_cols))
    te_arrays,te_rows,te_cols = X_test.shape
    X_train_new = np.reshape(X_train,(tr_arrays*tr_rows,tr_cols))
    print((X_train_new.shape))
    X_test_new = np.reshape(X_test,(te_arrays*te_rows,te_cols))
    print((X_test_new.shape))
    if mode == 'myMaxMin':
        tr_min = X_train_new.min(0)
        tr_ptp = X_train_new.max(0) - tr_min
        X_train_new = (X_train_new-tr_min)/tr_ptp
        X_test_new = (X_test_new-tr_min)/tr_ptp
    else:
        if mode == 'Standard':
            scaler = preprocessing.StandardScaler().fit(X_train_new)
        elif mode == 'Normalizer':
            scaler = preprocessing.Normalizer().fit(X_train_new)
        
        X_train_new = scaler.transform(X_train_new)
        X_test_new = scaler.transform(X_test_new)
    start = 2
    #print(X_train_new[0:start])
    #print(X_test_new[0:start])
    X_train_new = np.reshape(X_train_new,(tr_arrays,tr_rows,tr_cols))
    print((X_train_new.shape))
    X_test_new = np.reshape(X_test_new,(te_arrays,te_rows,te_cols))
    print((X_test_new.shape))
    return X_train_new,X_test_new
    
    
    
def standardizeDataVal(X_train,X_test,X_val,mode):
    tr_arrays,tr_rows,tr_cols = X_train.shape
    ##print((tr_arrays,tr_rows,tr_cols))
    te_arrays,te_rows,te_cols = X_test.shape
    val_arrays,val_rows,val_cols = X_val.shape
    X_train_new = np.reshape(X_train,(tr_arrays*tr_rows,tr_cols))
    print((X_train_new.shape))
    X_val_new = np.reshape(X_val,(val_arrays*val_rows,val_cols))
    print((X_train_new.shape))
    X_test_new = np.reshape(X_test,(te_arrays*te_rows,te_cols))
    print((X_test_new.shape))
    if mode == 'myMaxMin':
    ...
    start = 2
    print(X_train_new[0:start])
    print(X_val_new[0:start])
    print(X_test_new[0:start])
    X_train_new = np.reshape(X_train_new,(tr_arrays,tr_rows,tr_cols))
    print((X_train_new.shape))
    X_val_new = np.reshape(X_val_new,(val_arrays,val_rows,val_cols))
